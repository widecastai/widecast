...
async function wc(method, path, body) {
    ...
    if (resp.ok && ctype.startsWith("image/")) {
        ...
        return { __mcpBinary: true, mimeType: ctype, base64: buf.toString("base64") };
    }
    let data = {};
    try {
...
const TOOLS = [
    ...
    },
    {
        ...
        description: "Mint a no-login client link ('magic link') for the account and " +
...
const server = new Server({ name: "widecast-mcp", version: VERSION, title: "WideCast" }, { capabilities: { tools: {} }, instructions: WIDECAST_MCP_INSTRUCTIONS });
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    try {
        ...
        if (name === "widecast_send_client_link") {
            const body = { link_type: String(args.link_type ?? "") };
