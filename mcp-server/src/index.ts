...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      ...
      "  1) `mkdir -p ./.widecast-skill-<format-folder> && cd ./.widecast-skill-<format-folder>` (folder name follows `result.download.work_dir`)\n" +
      "  2) `curl -fsSL <result.download.zip_url> -o <filename>.zip`\n" +
      "  3) (optional) `shasum -a 256 <filename>.zip` should equal `result.download.sha256`\n" +
      "  4) `unzip -o <filename>.zip`\n" +
      "Then `Read('<work_dir>/<format-folder>/SKILL.md')` and follow its LOAD MAP. For `format='video'`, sub-modules are `method`/`formats`/`hooks`/`ctas`/`research_visuals`/`handoff` — read via `Read('<work_dir>/video-script-writing/<id>.md')`. For `format='blog'` and `format='social'`, the skill is a single SKILL.md today (no sub-modules); load and follow directly. Per-format zips: `video-script-writing.zip` (~30KB), `blog-writing.zip` (~5KB), `social-post-writing.zip` (~3KB).\n" +
      "**Why** — agent step-adherence (research-first, fictional-frame factual-core, image vetting, hand-off order) is measurably better when content is read from a local file than from a tool result. Local-file path forces full reading; tool-result path tends to be skimmed once. The skill content on disk is identical to the previous inline-content mode — only the delivery mechanism changed.\n" +
      "**ALWAYS-RETURNED `contract` field** (~1.4 KB) ships in every response — read it FIRST as the safety net for cross-cutting rules (selector, autonomous run, research-first, image discipline, hand-off order).\n" +
      "**HOST REQUIREMENTS** — your runtime MUST have local shell + curl + unzip + a `Read`/file tool. If not, fall back to the `contract` field and tell the user the writing skill needs a desktop+shell host. Zip hosted at `origin.widecast.ai` (Cloudflare-bypassed) with `?v=<mtime>` cache-bust. Server can flip back to inline-content with env `WIDECAST_WRITING_DELIVERY_MODE=inline_content`. Key-free.",
    inputSchema: {
      type: "object",
      required: ["format"],
      properties: {
  ...
  },
  {
    ...
    description:
      ...
      "**⚠ DOWNLOAD-ZIP DELIVERY MODE (experiment, 2026-06-30)**: this tool no longer returns skill content inline. The response carries (a) a `preload_tools` block and (b) a 4-step download instruction.\n" +
      "**(a) PRELOAD ALL WIDECAST TOOLS FIRST** — the response includes `preload_tools.one_shot_query` (a literal `select:widecast_video_data,widecast_scene_geometry,widecast_scene_inspector,widecast_modify_scene,widecast_search_broll,widecast_upload_asset,...` string). Copy it verbatim into your ToolSearch call's `query` param BEFORE running the download step. Some MCP hosts (Codex, Claude Code) default to narrow keyword ToolSearch and will silently drop critical tools like `widecast_modify_scene` if the initial search misses them; running this preload guarantees every edit-flow tool is in your schema window when the flow reaches it. Fallback for hosts without `select:` syntax: use `preload_tools.broad_query` (`widecast`) with `preload_tools.broad_max_results` (~30). The list is auto-derived from the live MCP registry so adding a new tool later is zero-config.\n" +
      "**(b) THEN DOWNLOAD the zip**:\n" +
      ...
      "  Then `Read('./.widecast-skill-video-editing/video-editing/SKILL.md')` and follow its LOAD MAP — each step names a sub-module that you load via `Read('.../ai_video_editor/<id>.md')` from disk.\n" +
      ...
      "**`preload_tools` ships in BOTH delivery modes** (download_zip and inline_content). Always look for it in the response; it's the future-proof way to load the entire WideCast toolkit regardless of your host's ToolSearch defaults.\n" +
  ...
  },
  {
    ...
    description:
      "Generate 1-4 AI images from a text prompt. SYNC. **Charges 1 credit per image generated** (count=4 → 4 credits). Same engine as the dashboard's Gen-AI tab (broll.js). " +
  ...
  },
  {
    ...
    description:
      "Search stock B-roll for a keyword. SYNC, FREE. Two modes: **`kind='video'`** searches Pexels + Pixabay + Shutterstock for stock CLIPS (broll.js Stock tab). **`kind='image'`** searches Google for real PHOTOS (broll.js Photos tab). " +
  ...
  },
  {
    ...
    description:
      "Open/close the per-video EDIT SESSION for an AI editing run. SYNC, FREE. " +
  ...
  },
  {
    ...
    description:
      "Edit ONE scene of an existing video in place. SYNCHRONOUS for most branches (some upload branches are async — see below), **NO credit charged** until widecast_export_video re-renders the final MP4. Successful edits publish MQTT realtime to every open scene editor so the user sees the change live.\n" +
      ...
      "**(B) Upload Overlay (FREE; agent-supplied asset → Remotion spec).** `[{field_name:'remotion.upload_overlay', value:'https://.../overlay.svg'}]` or `{value:{url:'https://...'}}`. NOT Regenerate Overlay (which is paid because WideCast calls image generation). **Two upload formats, chosen by URL extension**:\n" +
      "  • **📐 SVG path (PREFERRED for agent-authored overlays)** — URL ends `.svg`. WideCast routes through `svg2spec`: parses the SVG, finds every `<g data-wc-object=\"name\">` group, rasterizes EACH group ALONE on a transparent 720×1280 canvas, crops to the alpha bbox, emits one Storyboard object per group with **pixel-perfect placement (NO auto-fit; max diff 2/255 vs original)**. **Required**: viewBox `0 0 720 1280` (portrait). Per-group attributes: `data-wc-object` (REQUIRED — stable id; groups without this become background); `data-wc-kind` ∈ {`object`, `text`, `bar`, `mark`, `callout`} (inferred if missing); `data-wc-anim` ∈ {`slide-up`, `slide-down`, `slide-left`, `slide-right`, `pop`, `grow-x`, `grow-y`, `fade`, …} (default `fade`; bars get growRight/growUp); `data-wc-z` = paint/stack order (default = document order). Use this when you want **deterministic, no-classifier decomposition** with animation hints in one authored file. SVG errors → 422 `svg2spec:<reason>`; download errors → 502.\n" +
      "  • **🖼 Raster path (default)** — URL extension is PNG / JPG / WebP / GIF / etc. WideCast classifies the uploaded image as `graphic` vs `realistic`, decomposes graphic images into objects, falls back to a full-frame single-image overlay for realistic ones (strict no-AI fallback). For best object decomposition: portrait 9:16 transparent PNG (720×1280 preferred) or flat-bg graphic with large separated foreground objects/text, strong contrast, readable typography; avoid photo-realistic backgrounds, heavy gradients, vignettes, tiny dense text. When in doubt, author SVG — no classifier in the loop.\n" +
      "  If the agent creates the asset first, it MUST be grounded in scene context (`text`, `talking_point`, `visual`, `quote`, `keyword`, `type`). Upload Overlay is an explicit opt-in to recreate an overlay even when the scene had `remotion_spec='none'`. Response carries cache-busted `remotion_spec_url` + `remotion_poster_url` (static overlay poster used by server-side scene_inspector fallback).\n" +
      ...
      "**(M) Remotion add element (FREE; SYNC; add-only).** Send exactly one field `[{field_name:'remotion.add_element', value:{kind, value?, label?, url?, position?, rect?, style_token?, emphasis?}}]` — or the bare-string shortcut `value:'Quick caption'` for kind=text. `kind` ∈ {`text`, `stat`, `label`, `callout`, `image`} (aliases: `metric`/`number`→`stat`, `badge`/`caption`→`label`; defaults to `image` when a `url`/`image`/`src` is provided, else `text`). `value` = text content for text/stat/callout; `label` = secondary text for label/stat; `url` (or `image`/`src`) = http(s) image URL for kind=image. `position` ∈ {top, bottom, left, right, center} — server picks a sane default rect for that side; OR pass `rect:{x, y, w, h, coordinate_space:'preview'|'canvas'}` (default 280×498 preview) — top-level shorthand `x/y/w/h` on the value object also accepted. Optional `entry` (default `fade`), `delay_sec`, `z_index`, `element_z_index` (default 900). **ADD-ONLY**: appends a new Storyboard group/object to the existing spec — does NOT auto-fit, replace, move, or modify existing objects. Requires an enabled spec (returns 409 `remotion_spec_disabled` otherwise). Must be sent ALONE (400 `mixed_remotion_fields` if combined with other Remotion fields or used inside `layout.batch`). Response `applied` returns `kind`, `element_id`, `object_id`, `layout_id` (= `{element_id}.{object_id}`, e.g. `agent_add_01.add_text_1782700000000`), `rect` (preview), `rect_canvas` (720×1280), `auto_fit:false`, `cost:'free_spec_append'`, plus `follow_up` hint. **Recommended workflow**: immediately call widecast_scene_geometry to read the displayed rect + violations, then issue a `layout.batch` with a `remotion.object.rect` carrying the new `layout_id` to land the element inside safe zones — the initial placement is a default, not a polished layout. Errors: 400 `invalid_remotion_add_element` / `invalid_remotion_add_element_kind` / `mixed_remotion_fields`, 404 `remotion_spec_not_found`, 409 `remotion_spec_disabled` / `missing_voice_file` / `remotion_spec_parse_failed` / `remotion_spec_invalid`.\n" +
      "**(N) Disable overlay (FREE; SYNC; data edit).** `[{field_name:'disable_overlay', value:true}]` — aliases accepted: `remotion.disable_overlay`, `overlay.disable`, `remotion_spec.none`. `value` may be `true`, the string `'none'`, or `{enabled:false}`. **What it does**: sets `segment.remotion_spec='none'` (the opt-out sentinel) and emits MQTT `scene_modified` with `overlay_disabled:true`. **What it does NOT do**: it does NOT delete `{voice_file}_spec.json`, does NOT delete `{voice_file}_overlay_poster.png`, does NOT regenerate a replacement, and does NOT change narrator / caption / media / timing / layout. This is the API equivalent of a user deciding this scene should show no Remotion overlay. **Sent ALONE** (400 `mixed_remotion_fields` if combined with other fields). **Idempotent**: re-disabling a scene that is already `remotion_spec='none'` returns `applied.idempotent:true`. **Re-enable**: call `(B) remotion.upload_overlay` or the manual regenerate-overlay flow — those write a fresh `{voice_file}_spec.json` and stamp the segment's `remotion_spec` to `{voice_file}_spec.json?v={mtime}` so CF/browser caches fetch the new file. **Layout edits after disable** return 409 `remotion_spec_disabled` — that's expected; do NOT auto-enable. Errors: 400 `invalid_disable_overlay_value` (value not in {true, 'none', {enabled:false}, …}) / `mixed_remotion_fields`, 409 `missing_voice_file`.\n" +
      "**(O) Overlay TEXT edit (FREE; SYNC; 0-LLM; SVG posters only).** `[{field_name:'overlay.text_edit', value:{wc_object:'support_stack', lines:['NEW LINE 1','NEW LINE 2'], fill?:'#1E6091'}}]` (one or more entries, each targets one `<g data-wc-object>` group; `lines` may be a single string; `fill` is an optional hex colour that repaints the group's main text; send `lines` and/or `fill` — a fill-only edit keeps the existing words). Edits the TEXT and/or COLOUR of an existing SVG-sourced overlay withOUT re-authoring or re-uploading: WideCast rewrites `{voice_file}_overlay.svg` in place (face-stack copies update together; fewer lines delete slots, extra lines stack below), re-rasterizes ONLY the touched groups, and patches the existing spec objects — layout, narrator-fit shifts and manual positions on other objects are preserved. Requires `spec._source=='svg2spec'` (endpoint posters / uploaded SVG overlays); typography and image scenes return 422 `no_svg_overlay_source` (for typography, edit `quote` via (L) metadata and regenerate). Read group ids from widecast_scene_geometry object metadata (`wc_object`) or the `{voice_file}_overlay.svg` source. Poster copy rules (max 3 lines, max 4 words/line, uppercase) come back as `applied.warnings` only — the typed copy is sovereign, but heed the warnings: long lines shrink to fit the safe band. The `type='thumbnail'` clone of an edited opening poster is NOT auto-synced — apply the same overlay.text_edit to the thumbnail scene's voice_file. Sent ALONE (400 `mixed_remotion_fields`). Errors: 400 `invalid_text_edit` / `missing_text_edit`, 404 `remotion_spec_not_found`, 409 `remotion_spec_disabled` / `missing_voice_file`, 422 `no_svg_overlay_source`.\n" +
      "**`remotion_spec='none'`** = user intentionally disabled the overlay (via (N) Disable overlay or UI). Layout edits return `remotion_spec_disabled`. Do NOT auto-enable — only re-enable via (B) Upload Overlay if the user explicitly asks.\n" +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        fields: {
          ...
          minItems: 1,
          description:
            "Edits to apply. Pick exactly ONE family per call (layout.batch is the only multi-family composer; it only combines layout fields).\n" +
            "(A) Media: `[{field_name:'mediaUrl', value:'<URL>'}, {field_name:'mediaType', value:'image'|'video'}?]`.\n" +
            "(B) Upload overlay: `[{field_name:'remotion.upload_overlay', value:'<URL>'}]` (free, agent-supplied → Remotion spec). Supports raster (PNG/JPG/WebP/etc — classifier-driven decomposition) AND SVG (URL ends `.svg`, viewBox 720×1280, each `<g data-wc-object>` group becomes one Storyboard object pixel-perfectly).\n" +
            ...
            "(M) Add element (append-only): `[{field_name:'remotion.add_element', value:{kind:'text'|'stat'|'label'|'callout'|'image', value?, label?, url?, position?:'top'|'bottom'|'left'|'right'|'center', rect?:{x,y,w,h,coordinate_space?}, style_token?, emphasis?}}]` (or bare string for kind=text). Sent ALONE. Returns new `layout_id` — follow up with a `layout.batch` carrying a matching `remotion.object.rect` for safe-zone placement.\n" +
            "(N) Disable overlay: `[{field_name:'disable_overlay', value:true}]` (aliases: `remotion.disable_overlay` / `overlay.disable` / `remotion_spec.none`). Sets `segment.remotion_spec='none'` without deleting `{voice_file}_spec.json` / overlay poster. Sent ALONE. Re-enable later via (B) Upload Overlay.\n" +
            "(O) Text edit (SVG posters, 0-LLM): `[{field_name:'overlay.text_edit', value:{wc_object:'<g data-wc-object id>', lines?:['NEW LINE'], fill?:'#1E6091'}}]`. Rewrites {voice_file}_overlay.svg + patches the spec in place; `lines` retypes, `fill` (hex) recolours the group's main text (either or both); requires spec._source='svg2spec'; poster copy rules returned as warnings only. Sent ALONE.",
          items: {
            type: "object",
            required: ["field_name", "value"],
            properties: {
              field_name: { type: "string", description: "One of: mediaUrl, mediaType, remotion.upload_overlay, remotion.add_element, overlay.text_edit, disable_overlay (aliases: remotion.disable_overlay / overlay.disable / remotion_spec.none), remotion.object.rect, remotion.group.rect, overlay.narrator.rect, overlay.narrator.x|y|w|h, overlay.caption.y, layout.batch, voice.upload, narrator.upload_video, roll.active, roll.switch, segment.text, pattern, visual, keyword, quote, talking_point, type, sub_mode (or segment./scene. namespaced forms)." },
    ...
    outputSchema: {
      ...
      properties: {
        ...
        remotion_element_added: { type: "boolean", description: "True when the (M) Remotion add_element branch ran." },
        overlay_disabled: { type: "boolean", description: "True when the (N) Disable overlay branch ran — segment.remotion_spec was set to 'none' (no spec/poster files deleted)." },
  ...
  },
  {
    ...
    description:
      "Read structured video/scene data for a topic_id. SYNC, FREE. **First step for data-first scene audit/edit** — call this before widecast_scene_geometry / widecast_modify_scene / widecast_scene_inspector. The recommended chain is: **video_data → scene_geometry (for layout decisions) → modify_scene** (or use scene_inspector only as an expensive last resort when you need browser truth / a small live screenshot).\n" +
  ...
  },
  {
    ...
    description:
      "Report a WideCast problem to the WideCast team. SYNC, FREE. Use this " +
  ...
  // widecast.ai/#setup) — same policy as platform_settings save.
  {
    ...
    description:
      "List the account's CHANNEL GROUPS — separate sets of connected social accounts (e.g. 'Primary', 'Vietnamese channels', 'English channels'), each backed by its own publishing profile. Read-only, free. " +
  ...
  },
  {
    ...
    description:
      "Queue a new idea/topic into the account's PRODUCTION PLAN. SYNC, FREE. " +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        scripts: {
          ...
          maxItems: 5,
          description:
            "Optional — SCRIPT ATTACH. 1-5 pre-written script versions, one per " +
  ...
  {
    name: "widecast_send_client_link",
    title: "WideCast: Mint + send a no-login client link (magic link)",
    description:
      "Mint a no-login client link ('magic link') for the account and " +
      "OPTIONALLY send it through the account's notification channels. SYNC, " +
      "FREE. WHEN to use: after you finished a video, send the client a " +
      "`record` link so they can open that project's recording workspace; " +
      "after you queued an idea (widecast_add_to_production_plan), send a " +
      "`content_plan` link so the client can review the plan. `link_type` " +
      "(REQUIRED) is one of: `record` (opens ONE specific project's recording " +
      "workspace — REQUIRES `topic_id`), `content_plan` (the Saved Ideas / " +
      "content-plan screen), `setup` (the account Setup Center), " +
      "`social_dashboard` (the Social Dashboard — statistics), " +
      "`publish_schedule` (the Publish Schedule screen). `ttl_days` = link " +
      "lifetime in days, clamped to 1..30, default 7. `channels` = " +
      "`{telegram?, sms?, email?}` booleans; OMITTED, empty, or all-false => " +
      "MINT-ONLY — no notification is sent, you get `magic_url` back and " +
      "decide later. Recipients are ALWAYS resolved server-side from the " +
      "account's Setup > Notification settings — you can NEVER supply a phone " +
      "number or email address (anti-spam-relay design). Notification content " +
      "is composed server-side per link_type (record includes the video's " +
      "title/hook; content_plan says the content plan is ready for review) " +
      "across Telegram, SMS (Twilio) and email with a CTA button. Response " +
      "carries `magic_url`, `expires_in_days`, `notified`, and — only when " +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        channels: {
          type: "object",
          description: "Optional. Omitted, empty, or all-false = MINT-ONLY (no notification; caller gets magic_url and decides later). Recipients are always resolved server-side from Setup > Notification settings — a phone number or email address can never be passed in.",
          properties: {
            telegram: { type: "boolean", description: "Send via the account's connected Telegram." },
            sms: { type: "boolean", description: "Send via SMS (Twilio) to the account's configured phone." },
            email: { type: "boolean", description: "Send via email to the account's configured address." },
          },
        },
        page: { type: "string", enum: ["record.html", "record2.html"], default: "record.html", description: "Which app page hosts the link. Default record.html." },
      },
    },
    annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: false, idempotentHint: false },
    outputSchema: {
      type: "object",
      properties: {
        object: { type: "string", const: "client_link" },
        link_type: { type: "string", enum: ["record", "content_plan", "setup", "social_dashboard", "publish_schedule"] },
        magic_url: { type: "string", description: "The no-login link to hand to the client." },
        expires_in_days: { type: "integer" },
        notified: { type: "boolean" },
        topic_id: { type: "string", description: "Present only for link_type=record." },
        results: {
          type: "object",
          description: "Present only when channels were requested. Per-channel delivery status: sent | skipped | failed, with reason/error.",
          properties: {
            telegram: { type: "object", properties: { status: { type: "string", enum: ["sent", "skipped", "failed"] }, reason: { type: "string" }, error: { type: "string" } } },
            sms: { type: "object", properties: { status: { type: "string", enum: ["sent", "skipped", "failed"] }, reason: { type: "string" }, error: { type: "string" } } },
            email: { type: "object", properties: { status: { type: "string", enum: ["sent", "skipped", "failed"] }, reason: { type: "string" }, error: { type: "string" } } },
          },
        },
        request_id: { type: "string" },
      },
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  try {
    ...
    if (name === "widecast_create_image") {
      ...
      if (args.topic_id !== undefined) body.topic_id = args.topic_id;
    ...
    if (name === "widecast_video_data") {
      const body: Record<string, unknown> = { video_id: String(args.video_id ?? "") };
      if (args.include_diagnostics !== undefined) body.include_diagnostics = args.include_diagnostics;
      const data = await wc("POST", "/v1/video_data", body);
      return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    }
    if (name === "widecast_scene_geometry") {
      const body: Record<string, unknown> = { id: String(args.video_id ?? "") };
      if (args.by !== undefined) body.by = args.by;
      if (args.value !== undefined) body.value = args.value;
      if (args.voice_file !== undefined) body.voice_file = args.voice_file;
      if (args.scene_id !== undefined) body.scene_id = args.scene_id;
    ...
    if (name === "widecast_scene_inspector") {
      ...
      if (args.seek_seconds !== undefined) body.seek_seconds = args.seek_seconds;
    ...
    if (name === "widecast_modify_scene") {
      ...
      if (args.op !== undefined) body.op = args.op;
      if (args.min_score !== undefined) body.min_score = args.min_score;
    ...
    if (name === "widecast_report_error") {
      const body: Record<string, unknown> = { error_message: String(args.error_message ?? "") };
      if (args.module !== undefined && args.module !== "") body.module = args.module;
    ...
    if (name === "widecast_add_to_production_plan") {
      const body: Record<string, unknown> = { idea_text: String(args.idea_text ?? "") };
    ...
    if (name === "widecast_send_client_link") {
      const body: Record<string, unknown> = { link_type: String(args.link_type ?? "") };
      for (const k of ["topic_id", "ttl_days", "channels", "page"]) {
        if (args[k] !== undefined && args[k] !== "") body[k] = args[k];
      }
      const data = await wc("POST", "/v1/client_link/send", body);
      return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    }
