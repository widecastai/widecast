...
| `typography_only` | Text only — §5.2. On B-roll/faceless scenes, prefer the text block centered vertically inside `safe_rect`, not top-safe by habit. |
| `narration_only` | No overlay (`remotion_spec="none"`) — leave it. |

**Pattern text stroke policy.** Panel/card/checklist/table/timeline/map/callout/illustration text is secondary text unless it is the one scene title. It must pass the secondary text gate separately from the title: no visible stroke/outline on small text, no dark text on dark panels, no low-contrast label tucked inside an icon/card, and no overlaps between panel labels, badges, icons, and card copy. Pattern titles use the global title treatment: **open typography, 900-equivalent weight, no card/panel container, visible text stroke max 2px on the 720 canvas**. If a panel label is not readable at 280×498, simplify the words, enlarge it, use a clean chip/backplate, or rebuild the panel; do not declare PASS because the title looks good.
