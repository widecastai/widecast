...
1. **Scope is objective blind spots.** Check only what WideCast cannot reliably see: rendered/baked overlay text and background/media fit. Do not judge taste, polish, title punch, or aesthetic quality by default.
...
6. **Background QA is mandatory; background plate is conditional.** Run Gate 5 for every scene from the BEFORE composite. Pull/show the active plate only when the visible background needs closer judgment. Mark `PASS skip` only for full-canvas A-roll, force-grid, disabled, or hidden backgrounds.
...
- Did I run Gate 5 for every scene, and avoid pulling background plates only when the composite was enough?
