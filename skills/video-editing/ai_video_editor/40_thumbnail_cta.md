...
> **⚠ RETIRED — DORMANT ON PURPOSE (2026 lean model). Do NOT load. Do NOT delete.** Endpoint/poster authoring — opening-poster, thumbnail-sync, closing-CTA design — is no longer an agent gate; WideCast produces the overlays and all content scenes use the same source-text plus integrity/provenance rules. There is no surviving opening-poster check. This file is kept intentionally, unpromoted and unused, ONLY as a reference in case endpoint authoring is ever re-enabled. It is not in the LOAD MAP; the fail-open "load unknown modules" rule does NOT apply to a RETIRED module. Do not load it, cite it, or run its endpoint proofs.
