...
Rendered image typo/grammar check: <PASS|FAIL|N/A> — read exact words visible in the BEFORE screenshot/preview itself, including overlay text and generated/image-baked text; do not rely only on JSON/source fields
...
Rendered preview text check: <PASS|FAIL|pending post-upload> — if a preview is shown, the visible rendered title glyphs exactly match the approved title and have no typo/grammar/diacritic/glyph error
...
Rendered preview text check: <PASS|FAIL|pending post-upload> — if a preview is shown, all visible rendered labels/values match the approved strings and have no typo/grammar/diacritic/glyph error
...
Rendered text inventory from screenshot: <exact visible words as they appear in the image, not copied from JSON>
...
Typo/grammar/diacritic/glyph check: <PASS|FAIL> — no typo, grammar error, missing/wrong diacritic, malformed glyph, pseudo-text, wrong language, wrong casing, wrong number/currency/%/symbol, or wrong domain term
...
**Missing proof = Gate 7 FAIL** whenever any visible overlay/image-baked message text exists. Do not declare `Scene N: PASS` until the rendered screenshot text has been read from the image itself and this check passes.
