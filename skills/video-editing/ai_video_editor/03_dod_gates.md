...
### Gate 4 MODULE LOAD PROOF — before authoring/rebuilding SVG
...
**If this is A-roll and this proof is missing, Gate 4 is BLOCKED.** Do not accept an existing shrunken narrator layout, draw/upload an SVG, or mark overlay review PASS. If choosing priority 4, the proof must show concrete reasons priorities 1–3 failed.
...
**Any FAIL = title is not approved for upload.** Revise the SVG/title and repeat the gate. After upload, the final screenshot gate must still PASS from the saved/shown 280×498 composite:
