...
- **rendered image typo/grammar check passed from the clean overlay poster:** when visible overlay/message text exists, the agent called MCP `widecast_scene_inspector` with `action="overlay_poster"`, `id`, `voice_file`, and `activate:true`, downloaded/shown the returned URL as a local PNG for the current `remotion_spec`, and produced the Gate 7 **per-string transcription table** (one row per visible string, typed letter-by-letter from the poster — transcribe FIRST, judge style after; a summary line without the table means the check did not run), reading the exact visible words there, including generated/image-baked text, chart/map/document/UI labels, and old overlay assets; no pseudo-text, malformed glyph, stale spec text, missing/wrong diacritic, typo, grammar error, wrong language, wrong number/currency/%/symbol, or wrong domain term remains; for Vietnamese/diacritic languages, every tone mark, accent, horn/breve/circumflex, vowel mark, and `Đ/đ` is visibly correct. If the poster result was unavailable, the proof explicitly states the composite fallback. Manually constructed poster URLs do not satisfy this check.
- **title readability is only the minimum floor:** hero/title text must be bright, high-contrast, vivid, **900-weight and thick-bodied via an 8–15x same-fill face stack**, with a thin controlled outline <=2px, immediately prominent, graspable in the first second, and states the scene takeaway rather than merely repeating a panel label. Readable-but-thin, truly hollow-looking, grey, or timid title text fails even if every letter can technically be read; **true hollow means transparent/missing/weak interior fill, not a good `solid_outlined`/sticker title with an opaque filled face.** Also inspect for **over-thick duplicate fill**: if letters look blobby, counters/negative space close up, Vietnamese diacritics are swallowed, or the form loses premium sharpness, reduce stack count/offset or change font; it fails even within 8–15. More than 15 face copies is over-thick and fails.
...
- **Gate 9 MODULE COVERAGE GATE passed** — `00_ENTRYPOINT`, `01_critical_rules`, `02_jump_prevention`, `03_dod_gates`, `04_principles_workflow`, `05_quality_qa_priority`, `10_mechanics`, `20_background`, `30_overlay_core`, `31_typography`, `32_charts`, `33_patterns`, `styles/design_languages`, and `40_thumbnail_cta` are either loaded where required or explicitly N/A with reasons
