...
- **the modules required for the work actually done were loaded at this step** (background pass → `20_background`; endpoint scene → `40_thumbnail_cta`; overlay → `30_overlay_core` + `31_typography` whenever the overlay contains any text + the matching content module/style lib) — a step done "from memory" without opening its module does not count as PASS
