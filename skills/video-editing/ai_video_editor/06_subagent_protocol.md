...
- **MAIN AGENT** — orchestrator + SOLE WRITER. Owns the SCENE ROSTER + run_ledger file, the run digest, every `modify_scene`/thumbnail-sync/export interaction, and the final hand-off. Applies writes serially in roster order and confirms each save (Gate 8) before the next.
