...
>
> **Universal title rule across every language:** use a renderer-supported font that covers and correctly shapes the scene's writing system. Keep hero text open rather than boxed. Heavy weight, face stacking, casing, tracking, and outlines are optional techniques, not cross-script assumptions: use them only when they preserve counters, joins, combining marks, punctuation, and readability. Never exceed 15 face copies; use shadow, glow, scrim/gradient, or object-integrated placement when stacking would deform the script.
>
