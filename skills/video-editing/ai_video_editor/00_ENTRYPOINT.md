...
3. Pull `video_data` once; build the full script for context (§1–§2). Print the SCENE ROSTER and write the run_ledger file (one row per content scene, blank verdict, total count). Mark the FIRST content row `opening` — it gets the Gate 4 OPENING POSTER CHECK (an aesthetic pass on the hook frame).
