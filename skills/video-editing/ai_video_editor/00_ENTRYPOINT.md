...
3b. **Delegation check — DEFAULT, not optional:** if this runtime can spawn subagents (an Agent/Task tool exists) AND the video has ≥3 content scenes, `Read` `ai_video_editor/06_subagent_protocol.md` NOW and run the subagent pipeline: export steward data files, spawn read-only PREPARE subagents in a rolling pool of K, apply all writes yourself serially in roster order, then spawn VERIFY subagents. Record `delegation mode: subagent (K=<n>)` in the run_ledger. Only run inline (steps below, one scene at a time in this context) with a recorded fallback reason: `no subagent capability` / `≤2 content scenes` / `user asked single-agent`. Conditional-sounding wording elsewhere does not make delegation optional.
4. Work ONE scene at a time, scene 2 → last content scene, in one continuous pass. (Under subagent mode this rule governs the APPLY phase + verdict recording: writes and `Scene N` verdicts advance strictly one roster row at a time, while prepare/verify fan out per `06_subagent_protocol`.) The thumbnail is synced immediately after scene 2 and is NOT revisited at the end.
