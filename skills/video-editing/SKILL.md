...
Gate 1 does **not** clear overlay copy by itself. A caption may be fixed while a title/label/value baked into `remotion_spec` or a newly authored SVG still has a typo, grammar issue, missing diacritic, wrong currency, wrong number, or wrong term. Gate 5 must inventory and proofread the exact visible overlay strings again, and Gate 8 must confirm the final screenshot still shows correct copy.
