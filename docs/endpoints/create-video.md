...
| `video_url` | `video_url` — any public http(s) URL (direct file link OR YouTube / TikTok / Facebook), ≤5 min | **Auto-Edit** — the footage itself becomes the B-roll |
| `video_file` | a video file (multipart upload) | Auto-Edit, but you upload the file instead of linking it |
| `audio_url` | `audio_url` — any public http(s) URL (direct file link OR YouTube / TikTok / Facebook), ≤5 min | **Audio-to-Video** — the audio becomes the narration, the AI builds matching scenes. **The natural path for an AI agent forwarding a chat-attached voice memo:** drop it on a public host (transfer.sh, file.io, S3 / R2, your CDN, …) and pass that URL. |
...
| `media_too_long` | 400 | The media exceeds the 5-minute duration cap (all media — URL, upload, or base64). |
